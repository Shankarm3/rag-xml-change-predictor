...
def read_xml(path: str) -> str:
    ...
    Raises:
        Exception: If file cannot be read or contains invalid XML
    """
    try:
        # First read as bytes to handle encoding declarations properly
        with open(path, 'rb') as f:
            content_bytes = f.read()
            
        # Log the first 100 bytes for debugging
        # logger.debug(f"Reading file: {path}")
        # logger.debug(f"First 100 bytes: {content_bytes[:100]!r}")
            
        # Try to detect and handle encoding declaration
        if content_bytes.startswith(b'<?xml'):
            # Find the encoding declaration
            decl_end = content_bytes.find(b'?>')
            if decl_end > 0:
                # Extract the XML declaration
                decl = content_bytes[:decl_end + 2].decode('ascii', 'ignore')
                # Try to get encoding from declaration
                encoding = 'utf-8'  # default
                if 'encoding=' in decl.lower():
                    import re
                    match = re.search(r'encoding=["\']([^"\']+)["\']', decl, re.IGNORECASE)
                    if match:
                ...
                try:
                    ...
                    error_msg = f"Error parsing XML in {path} with encoding {encoding}: {str(e)}"
                    logger.error(error_msg)
                    logger.error(f"File content starts with: {content_bytes[:100]!r}")
                    raise ValueError(error_msg) from e
        
        # If no XML declaration or encoding issues, try utf-8 with error handling
        try:
            ...
            error_msg = f"Invalid XML in {path}: {str(e)}"
            logger.error(error_msg)
            logger.error(f"File content starts with: {content_bytes[:100]!r}")
            raise ValueError(error_msg) from e
        ...
        error_msg = f"Permission denied when reading file: {path}"
        logger.error(error_msg)
        raise PermissionError(error_msg) from e
    except Exception as e:
        error_msg = f"Error reading XML file {path}: {str(e)}"
