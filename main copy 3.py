...
class ChangeAnalyzer:
    ...
    def get_most_changed_tags(self, top_n: int = 3) -> List[Tuple[str, int]]:
        """Get the most frequently changed tags."""
        return sorted(
            self.tag_changes.items(), 
            key=lambda x: x[1], 
            reverse=True
        )[:top_n]
    ...
    try:
        ...
        for path, elem_info in elements.items():
            ...
            if path not in analyzer.changed_paths:
                print("Path not in changed paths", path)
                continue
                        ...
                        if old_part in current_text:
                            ...
                            predictions['suggested_changes'].append({
                                ...
                                'suggested_change': {
                                    'from': old_part,
                                    'to': new_text,
                                    'confidence': min(90, frequency * 20),
                                    'pattern': pattern,
                ...
                for path, elem_info in elements.items():
                    if elem_info['tag'] == tag:
                        predictions['potential_improvements'].append({
                            ...
                            'change_count': count,
                            'suggestion': f'This {tag} element was frequently modified in previous versions',
                            'confidence': min(80, count * 15)  # Scale count to confidence
                        })
                        break
...
def run_pipeline():
    ...
    if os.path.exists(test_file):
        ...
        print("Please place your test XML file in the data/v1/ directory as 'sample_test.xml'")
